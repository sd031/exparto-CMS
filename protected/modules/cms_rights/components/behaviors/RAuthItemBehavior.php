...
class RAuthItemBehavior extends CBehavior
{
	...
	*/
	public function getNameLink()
	{
		return CHtml::link($this->getNameText(), array(
			'/cms_rights/admin/authItem/update',
	...
	*/
	...
	{
		return CHtml::linkButton(Yii::t('backend', 'STR_DELETE'), array(
			'submit'=>array('/cms_rights/admin/authItem/delete', 'name'=>urlencode($this->owner->name)),
			'confirm'=>Yii::t('backend', 'MSG_ARE_YOU_SURE_YOU_WANT_TO_DELETE_THIS_OPERATION?'),
			'class'=>'delete-link',
	...
	*/
	public function getDeleteTaskLink()
	{
		return CHtml::linkButton(Yii::t('backend', 'STR_DELETE'), array(
			'submit'=>array('/cms_rights/admin/authItem/delete', 'name'=>urlencode($this->owner->name)),
			'confirm'=>Yii::t('backend', 'MSG_ARE_YOU_SURE_YOU_WANT_TO_DELETE_THIS_TASK?'),
			'class'=>'delete-link',
			'csrf'=>Yii::app()->request->enableCsrfValidation,
		));
	}
	...
	*/
	public function getDeleteRoleLink()
	{
		...
		if( $this->owner->name!==Rights::module()->superuserName )
		{
			return CHtml::linkButton(Yii::t('backend', 'STR_DELETE'), array(
				'submit'=>array('/cms_rights/admin/authItem/delete', 'name'=>urlencode($this->owner->name)),
				'confirm'=>Yii::t('backend', 'MSG_ARE_YOU_SURE_YOU_WANT_TO_DELETE_THIS_ROLE?'),
				'class'=>'delete-link',
				'csrf'=>Yii::app()->request->enableCsrfValidation,
			));
		}
	...
	*/
	...
	{
		return CHtml::linkButton(Yii::t('backend', 'STR_REMOVE'), array(
			'submit'=>array('/cms_rights/admin/authItem/removeChild', 'name'=>urlencode($this->owner->name), 'child'=>urlencode($this->parent->name)),
			'class'=>'remove-link',
			'csrf'=>Yii::app()->request->enableCsrfValidation,
		));
	}
	...
	*/
	...
	{
		return CHtml::linkButton(Yii::t('backend', 'STR_REMOVE'), array(
			'submit'=>array('/cms_rights/admin/authItem/removeChild', 'name'=>urlencode($this->parent->name), 'child'=>urlencode($this->owner->name)),
			'class'=>'remove-link',
			'csrf'=>Yii::app()->request->enableCsrfValidation,
		));
	}
	...
	*/
	...
	{
		return CHtml::linkButton(Yii::t('backend', 'STR_REMOVE'), array(
			'submit'=>array('/cms_rights/admin/assignment/revoke', 'id'=>$this->userId, 'name'=>urlencode($this->owner->name)),
			'class'=>'revoke-link',
			'csrf'=>Yii::app()->request->enableCsrfValidation,
		));
	}
	...
	*/
	public function getRevokePermissionLink(CAuthItem $role)
	{
		...
		return CHtml::link(Yii::t('backend', 'STR_REVOKE'), '#', array(
			'onclick'=>"
				jQuery.ajax({
					type:'POST',
					url:'".Yii::app()->controller->createUrl('/cms_rights/admin/authItem/revoke', array(
						'name'=>urlencode($role->name), 
						'child'=>urlencode($this->owner->name),
					))."',
					data:{ ajax:1 $csrf },
					success:function() {
						$('#permissions').load('".Yii::app()->controller->createUrl('/cms_rights/admin/authItem/permissions')."', { ajax:1 $csrf });
					}
				});
	...
	*/
	public function getAssignPermissionLink(CAuthItem $role)
	{
		...
		return CHtml::link(Yii::t('backend', 'STR_ASSIGN'), '#', array(
			'onclick'=>"
				jQuery.ajax({
					type:'POST',
					url:'".Yii::app()->controller->createUrl('/cms_rights/admin/authItem/assign', array(
						'name'=>urlencode($role->name), 
						'child'=>urlencode($this->owner->name),
					))."',
					data:{ ajax:1 $csrf },
	...
	*/
	public function getInheritedPermissionText($parents, $displayType=false)
	{
		...
		return '<span class="inherited-item" title="'.implode('<br />', $items).'">'.Yii::t('backend', 'STR_INHERITED').' *</span>';
	}
}
