...
class AuthItemController extends RController
{
	...
	*/
	...
	{
		Yii::app()->user->rightsReturnUrl = array('/cms_rights/admin/authItem/operations');
		...
		$dataProvider = new RAuthItemDataProvider('operations', array(
			...
			'sortable'=>array(
				...
				'url'=>$this->createUrl('/cms_rights/admin/authItem/sortable'),
			),
		));
	...
	*/
	...
	{
		Yii::app()->user->rightsReturnUrl = array('/cms_rights/admin/authItem/tasks');
	...
	*/
	...
	{
		Yii::app()->user->rightsReturnUrl = array('/cms_rights/admin/authItem/roles');
			...
			if( $model->validate()===true )
			{
				...
				if( ($generatedItems = $generator->run())!==false && $generatedItems!==array() )
				{
					Yii::app()->getUser()->setFlash($this->module->flashSuccessKey,
						Yii::t('backend', 'STR_AUTHORIZATION_ITEMS_CREATED')
					);
					$this->redirect(array('/cms_rights/admin/authItem/permissions'));
				}
			}
		}
		...
		if( isset($_POST['AuthItemForm'])===true )
		{
			...
			if( $formModel->validate()===true )
			{
				...
				Yii::app()->user->setFlash($this->module->flashSuccessKey,
					Yii::t('backend', 'STR_:name_CREATED.', array(':name'=>$item->getNameText()))
				);

				// Redirect to the correct destination
				$this->redirect(Yii::app()->user->getRightsReturnUrl(array('/cms_rights/admin/authItem/permissions')));
			}
		}
	...
	*/
	public function actionUpdate()
	{
		...
		if( isset($_POST['AuthItemForm'])===true )
		{
			...
			if( $formModel->validate()===true )
			{
				...
				Yii::app()->user->setFlash($this->module->flashSuccessKey,
					Yii::t('backend', 'STR_:name_UPDATED.', array(':name'=>$item->getNameText()))
				);
		...
		if( $childSelectOptions!==array() )
		{
			...
			if( isset($_POST['AuthChildForm'])===true )
			{
				...
				if( $childFormModel->validate()===true )
				{
					...
					Yii::app()->user->setFlash($this->module->flashSuccessKey,
						Yii::t('backend', 'STR_CHILD_:name_ADDED', array(':name'=>$child->getNameText()))
					);

					// Reidrect to the same page
					$this->redirect(array('/cms_rights/admin/authItem/update', 'name'=>urlencode($itemName)));
				}
			}
		}
		else
	...
	public function actionDelete()
	{
		...
		if( Yii::app()->request->isPostRequest===true )
		{
			...
			Yii::app()->user->setFlash($this->module->flashSuccessKey,
				Yii::t('backend', 'STR_:name_DELETED', array(':name'=>$item->getNameText()))
			);
		...
		{
			throw new CHttpException(400, Yii::t('backend', 'STR_INVALID_REQUEST'));
		}
	}
	...
	*/
	public function actionRemoveChild()
	{
		...
		if( Yii::app()->request->isPostRequest===true )
		{
			...
			Yii::app()->user->setFlash($this->module->flashSuccessKey,
				Yii::t('backend', 'STR_CHILD_:name_REMOVED.', array(':name'=>$child->getNameText()))
			);
	...
	*/
	...
	public function getType()
	{
		...
		if( in_array($type, $validTypes)===true )
			...
			throw new CException(Yii::t('backend', 'STR_INVALID_AUTHORIZATION_ITEM_TYPE'));
	}
	...
	*/
	public function loadModel()
	{
		if( $this->_model===null )
		{
			...
			if( $this->_model===null )
				throw new CHttpException(404, Yii::t('backend', 'STR_THE_REQUESTED_PAGE_DOES_NOT_EXIST'));
		}
