		    ...
		    if( isset($_POST['AssignmentForm'])===true )
			{
				...
				if( $formModel->validate()===true )
				{
					...
					Yii::app()->user->setFlash($this->module->flashSuccessKey,
						Yii::t('backend', 'STR_PERMISSION_:name_ASSIGNED', array(':name'=>$item->getNameText()))
					);

					$this->redirect(array('/cms_rights/admin/assignment/user', 'id'=>$model->getId()));
				}
			}
		}
		// No items available
	...
	*/
	public function actionRevoke()
	{
      ...
      die('disabled');
			...
			Yii::app()->user->setFlash($this->module->flashSuccessKey,
				Yii::t('backend', 'PERMISSION_:name_REVOKED.', array(':name'=>$item->getNameText()))
			);

			// if AJAX request, we should not redirect the browser
			if( isset($_POST['ajax'])===false )
				$this->redirect(array('/cms_rights/admin/assignment/user', 'id'=>$_GET['id']));
		}
		else
		{
